...
def test_golden(test_name):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if trace_file.exists() and schedule_file.exists():
            ...
            # Формируем команду
            cmd = [
                "python3", "machine_cli.py",
                str(tmp_bin),
                "--trace",
                "--trace-file", str(tmp_trace),
                "--ticks", str(meta.get("ticks", 10000)),
                "--data-words", str(meta.get("data_words", 1024))
            ]
            ...
            assert len(actual_lines) == len(expected_lines), f"Trace length mismatch: expected {len(expected_lines)}, got {len(actual_lines)}"
