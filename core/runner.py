...
def run_machine(code_bin_path: str, input_schedule: List[IOEvent], data_words: int, tick_limit: int, trace: bool = False, trace_file: str | None = None) -> Dict[int, List[int]]:
    ...
    cpu = CPU(code, data_words=data_words, io=io, tick_limit=tick_limit)
    trace_out = None
    if trace:
    ...
    try:
        while cpu.tick < cpu.tick_limit and not cpu._halted:
            if trace:
                line = f"t={cpu.tick} pc={cpu.pc} phase={cpu._phase} T={cpu.dp.t} Z={cpu.dp.z} AR={cpu.dp.ar} zero={int(cpu.dp.zero)} sign={int(cpu.dp.sign)} in_isr={int(cpu.in_isr)}\n"
                if trace_out:
