- # Лабораторная работа 4: Стековая машина
...
Детальный журнал выполнения показывает:
- `t` — номер такта
- `pc` — счётчик команд
- `phase` — фаза выполнения (FETCH_IR, LATCH_PC, EXEC)
- `T`, `Z` — регистры стека
- `AR` — адресный регистр
- `zero`, `sign` — флаги
- `in_isr` — флаг обработки прерывания
