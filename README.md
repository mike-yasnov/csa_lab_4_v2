...
- Ассоциативность/приоритет:
    - `*` выше `+ -`;
    - затем сравнения `<=`, `==`;
    - скобок для группировки выражений нет — полагайтесь на приоритет.
- Область видимости:
    - глобальная: все переменные видны во всех функциях;
    - функции `main` и `irqN` — точки входа/обработчики.
- Типизация:
    - статическая;
    - булевы значения представлены целыми `0/1`;
    - `char` хранится как число (8-бит) в 32-битном слове;
    - `long` — в двух соседних словах (lo/hi).
- Литералы:
    - целые десятичные (`INT`);
    - строковые (`"..."`);
    - строковый литерал допустим в `print("...")` и помещается как C-строка (NUL-терминатор).
- Встроенные функции:
    - `readInt()/readChar()/readString()/readLong()` — чтение из портов;
    - `printInt()/print()/printChar()/printLong()` — вывод;
    - `ei()/di()` — управление прерываниями; 
    - `set/get` — работа с массивами (переменная хранит указатель на буфер).
- Условие в `if/while`:
    - ноль — ложь;
    - любое ненулевое — истина.
